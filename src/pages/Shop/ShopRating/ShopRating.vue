      ...
      <div class="ratingselect">
        <div class="rating-type border-1px">
          ...
          <span class="block negative" :class="{active: selectType===1}" @click="setSelectType(1)">
                  不满意<span class="count">{{ shopRating.length - positiveSize }}</span>
                </span>
        </div>
        <div class="switch" :class="{on: onlyShowText}" @click="onlyShowText=!onlyShowText">
          <span class="iconfont icon-check_circle" v-show="onlyShowText"></span>
...
export default {
  ...
  computed: {
    ...
    filterRatings () {
      // 得到相关的数据
      const {shopRating, onlyShowText, selectType} = this

      // 产生一个过滤新数组
      return shopRating.filter(rating => {
